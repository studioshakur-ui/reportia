// src/lib/supabase.js
// -------------------------------------------------------------
// Client Supabase + sync cloud-first / offline-ready
// Variables requises (Netlify/locaux) :
//   VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY
// Tables attendues : plan, workers, tasks, impianti, activities, status, reports
// -------------------------------------------------------------
...
// Flush automatique quand le navigateur repasse online
...
export async function saveReport(row) {
  // (facultatif côté App.jsx, mais dispo si tu en as besoin)
...
export async function fetchWorkers() {
  if (!supabase) return null; // null => laisse l'app utiliser le cache local
  const { data, error } = await supabase
    .from("workers")
    .select("*")
